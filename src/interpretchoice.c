#include "headers/interpretchoice.h"
...
void choice(int choice, data* data){
    ...
    switch(choice){
        case 1:
            // Asks for the process ID then puts the cmd into the buffer with the taskkill cmd (check killProcess() in choiceWriter.c)
            input = prompt("Enter Process ID");
